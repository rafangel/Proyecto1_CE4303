...
#define THREAD_CNT 5//16
#define ITER_CNT 5//16
...
simple_thread_func (void *data_) 
{
  ...
  for (i = 0; i < ITER_CNT; i++) 
    {
	printf("\nin thread function... %d",data->id);
      //lock_acquire (data->lock);
      //*(*data->op)++ = data->id;
	data->iterations = data->iterations + 1;
	printf("\nhere goes my custom function, no lock... yolo: %d",data->iterations);
